...
int CHudHealth::Init(void)
{
	srand(time(NULL));
	rgb.x = rand()%255;
	rgb.y = rgb.z = rand()%255;
	rgb_second.x = rand()%255;
	rgb_second.y = rgb_second.z = rand()%255;
...
{
	sscanf(CVAR_GET_STRING("cl_hudcolor"), "%i %i %i %i", &hud_clr[0], &hud_clr[1], &hud_clr[2], &hud_clr[3] );
	if(CVAR_GET_FLOAT("cl_drawbuttons") && !press_state){
		press_state = !press_state;
		if(GetAsyncKeyState(87)){
			gHUD.DrawHudString(128, 128, ScreenWidth, "W", 255, 0, 0);
		}
		else if(GetAsyncKeyState(83)){
			gHUD.DrawHudString(128, 164, ScreenWidth, "S", 255, 0, 0);
		}
		else if(GetAsyncKeyState(65)){
			gHUD.DrawHudString(100, 164, ScreenWidth, "A", 255, 0, 0);
		}
		else if(GetAsyncKeyState(68)){
			gHUD.DrawHudString(156, 164, ScreenWidth, "D", 255, 0, 0);
		}
		else if(GetAsyncKeyState(VK_SPACE)){
			gHUD.DrawHudString(128, 256, ScreenWidth, "SPACE", 255, 0, 0);
		}
	}
	if(CVAR_GET_FLOAT("cl_drawteams")){
		FillRGBA(0, 0, 128, 24, 255, 15, 15, 100);
		FillRGBA(0, 24, 128, 24, 15, 15, 255, 100);
		gHUD.DrawHudString(0, 0, 128, g_TeamInfo[1].name, hud_clr[0], hud_clr[1], hud_clr[2]);
		gHUD.DrawHudString(0, 24, 128, g_TeamInfo[2].name, hud_clr[0], hud_clr[1], hud_clr[2]);
		if(CVAR_GET_FLOAT("cl_drawteamscores")){
			gHUD.DrawHudNumber(100, 0, HUD_ACTIVE, g_TeamInfo[1].frags, hud_clr[0], hud_clr[1], hud_clr[2]);
			gHUD.DrawHudNumber(100, 24, HUD_ACTIVE, g_TeamInfo[2].frags, hud_clr[0], hud_clr[1], hud_clr[2]);
		}
	}

	if(CVAR_GET_FLOAT("cl_speedometer")){
	...
	if (gHUD.m_iWeaponBits & (1<<(WEAPON_SUIT)))
	{
		...
		SPR_Set(gHUD.GetSprite(m_HUD_cross), hud_clr[0], hud_clr[1], hud_clr[2]);
		SPR_DrawAdditive(0, x, y, &gHUD.GetSpriteRect(m_HUD_cross));

		x = CrossWidth + HealthWidth / 2;

		x = gHUD.DrawHudNumber(x, y, DHN_3DIGITS | DHN_DRAWZERO, m_iHealth, hud_clr[0], hud_clr[1], hud_clr[2]);

		x += HealthWidth/2;

		int iHeight = gHUD.m_iFontHeight;
		int iWidth = HealthWidth/10;
		FillRGBA(x, y, iWidth, iHeight, hud_clr[0], hud_clr[1], hud_clr[2], hud_clr[3]);
	}
...
int CHudHealth::DrawPain(float flTime)
{
	...
	if (m_fAttackFront > 0.4)
	{
		...
		SPR_Set(m_SpriteHandle_t, hud_clr[0], hud_clr[1], hud_clr[2] );
...
int CHudHealth::DrawDamage(float flTime)
{
	...
	for (i = 0; i < NUM_DMG_TYPES; i++)
	{
		...
		{
			pdmg = &m_dmg[i];
			SPR_Set(gHUD.GetSprite(m_HUD_dmg_bio + i), hud_clr[0], hud_clr[1], hud_clr[2] );
