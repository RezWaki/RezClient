...
int CHudAmmoSecondary :: Draw(float flTime)
{
	...
	sscanf(CVAR_GET_STRING("cl_hudcolor"), "%i %i %i %i", &sec_ammo_clr[0], &sec_ammo_clr[1], &sec_ammo_clr[2], &sec_ammo_clr[3] );
	...
	if ( m_HUD_ammoicon )
	{
		...
		SPR_Set( gHUD.GetSprite(m_HUD_ammoicon), sec_ammo_clr[0], sec_ammo_clr[1], sec_ammo_clr[2] );
	...
	for ( int i = MAX_SEC_AMMO_VALUES-1; i >= 0; i-- )
	{
		...
		gHUD.DrawHudNumber( x, y, DHN_DRAWZERO, m_iAmmoAmounts[i], sec_ammo_clr[0], sec_ammo_clr[1], sec_ammo_clr[2] );

		if ( i != 0 )
		{
			// draw the divider bar
			x -= (AmmoWidth / 2);
			FillRGBA(x, y, (AmmoWidth/10), gHUD.m_iFontHeight, sec_ammo_clr[0], sec_ammo_clr[1], sec_ammo_clr[2], sec_ammo_clr[3]);
		}
	}
