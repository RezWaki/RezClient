...
int scr_r, scr_g, scr_b, scr_a;

int HUD_IsGame( const char *game );
int EV_TFC_IsAllyTeam( int iTeam1, int iTeam2 );

void GetScrColors( void ) {
	sscanf(CVAR_GET_STRING("cl_scoreboardcolor"), "%s %s %s %s", &scr_r, &scr_g, &scr_b, &scr_a );
}
...
void ScorePanel::Update()
{
	...
	{
		char sz[MAX_SERVERNAME_LENGTH + 64];
		sprintf(sz, "%s - %d", gViewPort->m_szServerName, CVAR_GET_STRING("name") );
