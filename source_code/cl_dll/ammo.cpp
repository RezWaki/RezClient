...
int CHudAmmo::Draw(float flTime)
{
	...
	sscanf(CVAR_GET_STRING("cl_hudcolor"), "%i %i %i %i", &ammo_clr[0], &ammo_clr[1], &ammo_clr[2], &ammo_clr[3] );
	...
	if (m_pWeapon->iAmmoType > 0)
	{
		...
		if (pw->iClip >= 0)
		{
			...
			x = gHUD.DrawHudNumber(x, y, iFlags | DHN_3DIGITS, pw->iClip, ammo_clr[0], ammo_clr[1], ammo_clr[2]);
			...
			ScaleColors(r, g, b, a );
			x = gHUD.DrawHudNumber(x, y, iFlags | DHN_3DIGITS, gWR.CountAmmo(pw->iAmmoType), ammo_clr[0], ammo_clr[1], ammo_clr[2]);		
	...
	if (pw->iAmmo2Type > 0) 
	{
		...
		if ((pw->iAmmo2Type != 0) && (gWR.CountAmmo(pw->iAmmo2Type) > 0))
		{
			...
			x = ScreenWidth - 4 * AmmoWidth - iIconWidth;
			x = gHUD.DrawHudNumber(x, y, iFlags|DHN_3DIGITS, gWR.CountAmmo(pw->iAmmo2Type), ammo_clr[0], ammo_clr[1], ammo_clr[2]);
...
int DrawBar(int x, int y, int width, int height, float f)
{
	...
	FillRGBA(x, y, width, height, ammo_clr[0], ammo_clr[1], ammo_clr[2], ammo_clr[3]);
	...
	for (i = 0; i < MAX_WEAPON_SLOTS; i++)
	{
		...
		if ( i == iActiveSlot )
		{
			...
			for ( int iPos = 0; iPos < MAX_WEAPON_POSITIONS; iPos++ )
			{
				...
				if ( gpActiveSel == p )
				{
					SPR_Set(p->hActive, ammo_clr[0], ammo_clr[1], ammo_clr[2] );
					SPR_DrawAdditive(0, x, y, &p->rcActive);

					SPR_Set(gHUD.GetSprite(m_HUD_selection), ammo_clr[0], ammo_clr[1], ammo_clr[2] );
				...
				else
				{
					...
					SPR_Set( p->hInactive, ammo_clr[0], ammo_clr[1], ammo_clr[2] );
		...
		else
		{
			...
			for ( int iPos = 0; iPos < MAX_WEAPON_POSITIONS; iPos++ )
			{
				...
				FillRGBA( x, y, giBucketWidth, giBucketHeight, ammo_clr[0], ammo_clr[1], ammo_clr[2], ammo_clr[3] );
