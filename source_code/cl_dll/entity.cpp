...
int DLLEXPORT HUD_AddEntity( int type, struct cl_entity_s *ent, const char *modelname )
{
	...
	if(!cvars_defined){
		...
		CVAR_CREATE("cl_doubleduck", "0", NULL);
		CVAR_CREATE("cl_drawbuttons", "0", NULL);
		CVAR_CREATE("cl_255fps", "0", NULL);
		CVAR_CREATE("cl_drawteams", "1", NULL);
		CVAR_CREATE("cl_specwh", "0", NULL);
		CVAR_CREATE("cl_minmodels", "0", NULL);
		CVAR_CREATE("cl_minmodelfile", "helmet", NULL);
